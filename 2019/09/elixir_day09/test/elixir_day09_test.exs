defmodule ElixirDay09Test do
  ...
  test "amplify_once" do
    ...
    |> Enum.each(fn {prog, phase_seq, want_val} ->
      got_val = ElixirDay09.amplify_once(prog, phase_seq)
    ...
    |> Enum.each(fn {prog, want_seq, want_val} ->
      {got_seq, got_val} = ElixirDay09.amplify_once_max_seq(prog)
  ...
  test "amplify_loop" do
    ...
    test_cases
    |> Enum.each(fn {prog, phase_seq, want_val} ->
      got_val = ElixirDay09.amplify_loop(prog, phase_seq)
      assert got_val == want_val
    end)
    ...
    test_cases
    |> Enum.each(fn {prog, want_seq, want_val} ->
      {got_seq, got_val} = ElixirDay09.amplify_loop_max_seq(prog)
      assert got_seq == want_seq
      assert got_val == want_val
    end)
  end
  ...
  test "day 5 part1 GenServer (Full Input)" do
    program = ElixirDay09.parse("../../05/input.txt")
  ...
  test "day 5 part1" do
    got =
      ElixirDay09.parse("../../05/input.txt")
      |> Computer.solve([1])
