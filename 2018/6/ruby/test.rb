...
coords.each do |coord|
  ...
  0.upto(BOARD_SIZE_X) do |x|
    0.upto(BOARD_SIZE_Y) do |y|
      if board[x][y][:assigned_coord] == i
        size += 1
        is_infinite = true if board[x][y][:is_inf]
      end
    end
  end
  ...
  area_calcs.push(
    { 'coord' => i, 'size' => size, 'is_infinite' => is_infinite }
  )
  i += 1
end
...
0.upto(BOARD_SIZE_Y) do |y|
  0.upto(BOARD_SIZE_X) do |x|
    ...
    coords.each { |coord| total_distance += distance(x, y, coord[0], coord[1]) }

    safe_squares += 1 if total_distance < SAFE_THRES
  end
end
